 ...
 */
public class SimplegtDebuggableInterpreter<ResultType, ContextType> extends be.ac.vub.simplegt.resource.simplegt.debug.AbstractSimplegtDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private be.ac.vub.simplegt.resource.simplegt.util.AbstractSimplegtInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private be.ac.vub.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public SimplegtDebuggableInterpreter(be.ac.vub.simplegt.resource.simplegt.util.AbstractSimplegtInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new be.ac.vub.simplegt.resource.simplegt.ISimplegtInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = be.ac.vub.simplegt.resource.simplegt.util.SimplegtStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public be.ac.vub.simplegt.resource.simplegt.util.AbstractSimplegtInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		be.ac.vub.simplegt.resource.simplegt.ISimplegtLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private be.ac.vub.simplegt.resource.simplegt.ISimplegtLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		be.ac.vub.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(be.ac.vub.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = be.ac.vub.simplegt.resource.simplegt.util.SimplegtEObjectUtil.getDepth(current);
		stopCondition = new be.ac.vub.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = be.ac.vub.simplegt.resource.simplegt.util.SimplegtEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = be.ac.vub.simplegt.resource.simplegt.util.SimplegtEObjectUtil.getDepth(current) - 1;
		stopCondition = new be.ac.vub.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
