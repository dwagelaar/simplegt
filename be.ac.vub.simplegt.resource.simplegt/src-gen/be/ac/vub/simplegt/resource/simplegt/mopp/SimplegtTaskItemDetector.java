...
public class SimplegtTaskItemDetector {
	...
	public java.util.List<be.ac.vub.simplegt.resource.simplegt.mopp.SimplegtTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<be.ac.vub.simplegt.resource.simplegt.mopp.SimplegtTaskItem> foundItems = new java.util.ArrayList<be.ac.vub.simplegt.resource.simplegt.mopp.SimplegtTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new be.ac.vub.simplegt.resource.simplegt.mopp.SimplegtTaskItem(keyword, message, localLine, offset, end));
