...
public class SimplegtDebugTarget extends be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugElement implements org.eclipse.debug.core.model.IDebugTarget, be.ac.vub.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener {
	
	/**
	 * associated system process
	 */
	private be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugProcess process;
	...
	private be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<be.ac.vub.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener> eventListeners = new java.util.ArrayList<be.ac.vub.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener>();
	
	private be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new be.ac.vub.simplegt.resource.simplegt.mopp.SimplegtMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((be.ac.vub.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public SimplegtDebugTarget(be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(be.ac.vub.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(be.ac.vub.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new be.ac.vub.simplegt.resource.simplegt.mopp.SimplegtMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					be.ac.vub.simplegt.resource.simplegt.debug.SimplegtLineBreakpoint lineBreakpoint = (be.ac.vub.simplegt.resource.simplegt.debug.SimplegtLineBreakpoint) breakpoint;
	...
	public void handleMessage(be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugMessage message) {
		try {
			if (message.hasType(be.ac.vub.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(be.ac.vub.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(be.ac.vub.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(be.ac.vub.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.RESUMED)) {
	...
	public be.ac.vub.simplegt.resource.simplegt.debug.SimplegtDebugProxy getDebugProxy() {
