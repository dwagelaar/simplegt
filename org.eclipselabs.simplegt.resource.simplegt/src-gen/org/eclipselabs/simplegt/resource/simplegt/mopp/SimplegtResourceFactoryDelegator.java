...
public class SimplegtResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	...
	protected void init() {
		...
		if (new org.eclipselabs.simplegt.resource.simplegt.util.SimplegtRuntimeUtil().isEclipsePlatformAvailable()) {
			new org.eclipselabs.simplegt.resource.simplegt.util.SimplegtEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtResourceFactory());
		}
	}
