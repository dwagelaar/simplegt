...
/**
 * The SimplegtTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * SimplegtTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class SimplegtTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtTaskItem> foundItems = new java.util.ArrayList<org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtTaskItem(keyword, message, localLine, offset, end));
