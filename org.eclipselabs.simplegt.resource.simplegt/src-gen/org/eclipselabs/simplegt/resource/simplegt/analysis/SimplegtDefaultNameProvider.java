...
public class SimplegtDefaultNameProvider implements org.eclipselabs.simplegt.resource.simplegt.ISimplegtNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.eclipselabs.simplegt.resource.simplegt.util.SimplegtEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
