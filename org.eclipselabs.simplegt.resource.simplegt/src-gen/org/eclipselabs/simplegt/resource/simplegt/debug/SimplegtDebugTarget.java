...
public class SimplegtDebugTarget extends org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugElement implements org.eclipse.debug.core.model.IDebugTarget, org.eclipselabs.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener {
	
	/**
	 * associated system process
	 */
	private org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugProcess process;
	...
	private org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<org.eclipselabs.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener> eventListeners = new java.util.ArrayList<org.eclipselabs.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener>();
	
	private org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((org.eclipselabs.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public SimplegtDebugTarget(org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(org.eclipselabs.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(org.eclipselabs.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtLineBreakpoint lineBreakpoint = (org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtLineBreakpoint) breakpoint;
	...
	public void handleMessage(org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugMessage message) {
		try {
			if (message.hasType(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.RESUMED)) {
	...
	public org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugProxy getDebugProxy() {
