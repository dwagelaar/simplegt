 ...
 */
public class SimplegtDebuggableInterpreter<ResultType, ContextType> extends org.eclipselabs.simplegt.resource.simplegt.debug.AbstractSimplegtDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.eclipselabs.simplegt.resource.simplegt.util.AbstractSimplegtInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.eclipselabs.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public SimplegtDebuggableInterpreter(org.eclipselabs.simplegt.resource.simplegt.util.AbstractSimplegtInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.eclipselabs.simplegt.resource.simplegt.ISimplegtInterpreterListener() {
	...
	public org.eclipselabs.simplegt.resource.simplegt.util.AbstractSimplegtInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.eclipselabs.simplegt.resource.simplegt.ISimplegtLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.eclipselabs.simplegt.resource.simplegt.ISimplegtLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.eclipselabs.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.eclipselabs.simplegt.resource.simplegt.util.SimplegtEObjectUtil.getDepth(current);
		stopCondition = new org.eclipselabs.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.eclipselabs.simplegt.resource.simplegt.util.SimplegtEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.eclipselabs.simplegt.resource.simplegt.util.SimplegtEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.eclipselabs.simplegt.resource.simplegt.ISimplegtCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
