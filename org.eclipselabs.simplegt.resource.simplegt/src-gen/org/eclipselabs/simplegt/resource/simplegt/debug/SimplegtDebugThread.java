...
public class SimplegtDebugThread extends org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugElement implements org.eclipse.debug.core.model.IThread, org.eclipselabs.simplegt.resource.simplegt.debug.ISimplegtDebugEventListener {
	
	private boolean suspended = false;
	private org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugTarget debugTarget;
	
	public SimplegtDebugThread(org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.eclipselabs.simplegt.resource.simplegt.util.SimplegtStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugTarget getTarget() {
		return debugTarget;
	}
