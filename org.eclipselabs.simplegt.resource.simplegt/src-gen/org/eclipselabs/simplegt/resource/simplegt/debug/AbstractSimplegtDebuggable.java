...
public abstract class AbstractSimplegtDebuggable {
	 ...
	 */
	private java.util.List<org.eclipselabs.simplegt.resource.simplegt.util.SimplegtPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.eclipselabs.simplegt.resource.simplegt.util.SimplegtPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new org.eclipselabs.simplegt.resource.simplegt.util.SimplegtRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new org.eclipselabs.simplegt.resource.simplegt.util.SimplegtRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugMessage message = new org.eclipselabs.simplegt.resource.simplegt.debug.SimplegtDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.eclipselabs.simplegt.resource.simplegt.util.SimplegtPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.eclipselabs.simplegt.resource.simplegt.util.SimplegtPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.eclipselabs.simplegt.resource.simplegt.util.SimplegtPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.eclipselabs.simplegt.resource.simplegt.debug.ESimplegtDebugMessageTypes.STARTED, true);
			setSuspend(true);
