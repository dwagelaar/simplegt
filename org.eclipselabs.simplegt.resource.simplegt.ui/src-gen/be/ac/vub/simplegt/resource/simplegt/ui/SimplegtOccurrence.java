...
public class SimplegtOccurrence {
	...
	private be.ac.vub.simplegt.resource.simplegt.ui.SimplegtTokenScanner tokenScanner;
	 ...
	 */
	public SimplegtOccurrence(org.eclipselabs.simplegt.resource.simplegt.ISimplegtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, be.ac.vub.simplegt.resource.simplegt.ui.SimplegtTokenScanner tokenScanner) {
	 ...
	 */
	public void handleOccurrenceHighlighting(be.ac.vub.simplegt.resource.simplegt.ui.SimplegtBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, be.ac.vub.simplegt.resource.simplegt.ui.SimplegtPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, be.ac.vub.simplegt.resource.simplegt.ui.SimplegtPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, be.ac.vub.simplegt.resource.simplegt.ui.SimplegtPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == be.ac.vub.simplegt.resource.simplegt.ui.SimplegtPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(be.ac.vub.simplegt.resource.simplegt.ui.SimplegtOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(be.ac.vub.simplegt.resource.simplegt.ui.SimplegtOccurrence.DECLARATION_ANNOTATION_ID);
	}
