 ...
 */
public class SimplegtOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.eclipselabs.simplegt.resource.simplegt.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.eclipselabs.simplegt.resource.simplegt.ui.occurences.declaration";
	...
	private org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtTokenScanner tokenScanner;
	...
	private org.eclipselabs.simplegt.resource.simplegt.ISimplegtTextResource textResource;
	 ...
	 */
	public SimplegtOccurrence(org.eclipselabs.simplegt.resource.simplegt.ISimplegtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.eclipselabs.simplegt.resource.simplegt.mopp.SimplegtMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.eclipselabs.simplegt.resource.simplegt.ui.SimplegtOccurrence.DECLARATION_ANNOTATION_ID);
	}
